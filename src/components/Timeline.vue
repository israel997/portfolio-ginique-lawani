<template>
  <section id="timeline" class="mb-16 md:mb-32 relative overflow-hidden px-4 md:px-0">
    ...
    <div class="relative max-w-4xl mx-auto">
      ...
      <div v-for="(item, index) in timelineItems" :key="item.id" class="relative mb-12 md:flex md:justify-between items-center group">
        ...
        <div :class="[
          ...
          <span class="text-pink-400 text-xs md:text-sm font-mono font-bold">{{ item.year_label }}</span>
          <h3 class="text-lg md:text-xl font-bold mt-1">{{ localized(item, 'title') }}</h3>
          <p class="text-indigo-300 text-xs md:text-sm mb-2 md:mb-3">{{ localized(item, 'company') }}</p>
...
const timelineItems = computed(() => content.value.timeline || []);
</script>
...
/* Animation de flottage désactivée sur mobile pour les performances */
