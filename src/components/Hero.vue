<template>
  <header id="hero" class="text-center min-h-[70vh] flex flex-col justify-center items-center mb-16 md:mb-32 relative px-4" role="banner">
    ...
    <h1 class="text-3xl sm:text-5xl md:text-8xl font-black mb-6 tracking-tight leading-tight px-2">
      <span itemprop="name">{{ nameFirstPart }} <span class="text-transparent bg-clip-text bg-gradient-to-r from-pink-400 via-purple-400 to-indigo-400 animate-pulse">{{ nameLastPart }}</span></span>
    </h1>

    <p class="text-sm sm:text-base md:text-lg text-slate-400 max-w-2xl mb-8 sm:mb-10 leading-relaxed px-2">
      <span class="text-white font-semibold">{{ localized(settings, 'hero_intro') }}</span>, {{ localized(settings, 'hero_intro2') }}
    </p>
    ...
    <div class="flex flex-col md:flex-row gap-3 md:gap-4 mb-12 w-full md:w-auto px-2 md:px-0 items-center">
      ...
      <a :href="settings.cv_url" target="_blank" class="px-4 md:px-6 py-2.5 bg-gradient-to-r from-indigo-600 to-blue-600 rounded-xl font-bold text-sm hover:shadow-lg hover:shadow-blue-500/40 hover:-translate-y-1 transition-all duration-300 flex items-center justify-center gap-2 hover:scale-105 zoom-mobile-2 min-w-[180px]">
      ...
      <img v-if="settings.hero_badge_image_url" :src="settings.hero_badge_image_url" alt="NASA Space Apps Challenge" loading="lazy" class="max-w-5xl w-[98%] md:w-full rounded-xl md:rounded-2xl shadow-2xl border border-white/10 hover:scale-105 transition-transform duration-300" />
    </div>
  </header>
</template>
...
const nameFirstPart = computed(() => {
  const name = settings.value.full_name || '';
  const parts = name.trim().split(' ');
  return parts.slice(0, -1).join(' ');
});
const nameLastPart = computed(() => {
  const name = settings.value.full_name || '';
  const parts = name.trim().split(' ');
...
const phrases = computed(() => [
  localized(settings.value, 'typewriter1'),
  localized(settings.value, 'typewriter2')
].filter(Boolean));
